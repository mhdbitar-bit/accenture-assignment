...
extension LocalCategoryLoader: CategoryLoader {
    ...
    func load(completion: @escaping (LoadResult) -> Void) {
        store.retrieve { [weak self] result in
            ...
            case let .found(categories, timestamp) where self.cachePloicy.validate(timestamp, against: self.currentDate()):
...
extension LocalCategoryLoader {
    func validateCahce() {
        store.retrieve { [weak self] result in
            ...
            case let .found(_, timestamp) where !self.cachePloicy.validate(timestamp, against: self.currentDate()):
                self.store.deleteCachedCategories { _ in }
